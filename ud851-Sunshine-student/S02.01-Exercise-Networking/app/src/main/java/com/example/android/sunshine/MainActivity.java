...
public class MainActivity extends AppCompatActivity {
    ...
    public class FetchWeatherTask extends AsyncTask<String, Void, String[]>{
        ...
        protected String[] doInBackground(String... params) {
            String[] resultWeatherJSONArray = null;
            URL url = NetworkUtils.buildUrl(params[0]);
            try {
                String resultWeatherJSONString = NetworkUtils.getResponseFromHttpUrl(url);
                resultWeatherJSONArray = OpenWeatherJsonUtils.getSimpleWeatherStringsFromJson(MainActivity.this, resultWeatherJSONString);
            } catch (IOException e) {
                e.printStackTrace();
            } catch (JSONException e) {
                e.printStackTrace();
            }
            return resultWeatherJSONArray;
        }
        // COMPLETE (7) Override the onPostExecute method to display the results of the network request
        @Override
        protected void onPostExecute(String[] params) {
            if(params != null) {
                for(String weatherData : params){
